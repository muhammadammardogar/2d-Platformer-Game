...
public class HeroKnight : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (Input.GetMouseButtonDown(0) && m_timeSinceAttack > 0.25f && !m_rolling)
        {
            ...
            // Check for nearby enemies in range and deal damage
    ...
    // Method to check for nearby enemies in range and apply damage
    private void CheckForEnemiesInRange()
    {
        // Find all enemies in the scene (can be optimized with a specific enemy pool or tagging)
        ...
        foreach (var enemy in enemies)
        {
            if (enemy != null)
            {
                ...
                // Apply damage if the enemy is within range
                if (distance <= m_attackRange)
                {
                    enemy.TakeDamage(m_attackDamage);  // Apply damage to the enemy
                    Debug.Log("Enemy hit within attack range!");
                }
            }
        }
    }
